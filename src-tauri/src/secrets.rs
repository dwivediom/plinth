//! OS-keychain `SecretStore` for the desktop app.
//!
...
//!
//! Keep in sync with `crates/mcp/src/secrets.rs`, which duplicates this so
//! the MCP binary reads the same keychain entries without a shared crate.
